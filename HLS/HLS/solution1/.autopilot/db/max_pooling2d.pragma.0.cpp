# 1 "../layers_c/max_pooling2d.cpp"
# 1 "../layers_c/max_pooling2d.cpp" 1
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 155 "<built-in>" 3
# 1 "<command line>" 1
...
# 3 "../layers_c/max_pooling2d.cpp" 2





uint8_t max_pooling2d_fix16(uint16_t kernel_size, uint16_t input_depth,
  uint16_t input_height, uint16_t input_width, int16_t* input,
  uint16_t output_depth, uint16_t output_height, uint16_t output_width,
  int16_t* output) {
...
#pragma HLS PIPELINE
 for (uint16_t in_h = 0; in_h < kernel_size; in_h++) {
     for (uint16_t in_w = 0; in_w < kernel_size; in_w++) {
      tmp = input[out_d * input_height * input_width
        + (kernel_size * out_h + in_h) * input_width
        + (kernel_size * out_w + in_w)];
    ...
    output[out_d * output_height * output_width
      + out_h * output_width + out_w] = buffer;
   }
  }
 }
...
uint8_t max_pooling2d_float32(uint16_t kernel_size, uint16_t input_depth,
  uint16_t input_height, uint16_t input_width, float* input,
  uint16_t output_depth, uint16_t output_height, uint16_t output_width,
  float* output) {
     ...
     for (uint16_t in_w = 0; in_w < kernel_size; in_w++) {
      if (in_h == 0 && in_w == 0) {
       ...
       output[out_d * output_height * output_width
         ...
         input[out_d * input_height * input_width
           + (kernel_size * out_h + in_h)
             * input_width
           + (kernel_size * out_w + in_w)];

      } else if (output[out_d * output_height * output_width
        + out_h * output_width + out_w]
        < input[out_d * input_height * input_width
          + (kernel_size * out_h + in_h)
            * input_width
